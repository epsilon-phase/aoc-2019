...
let copy_moon m=
  {pos=copy_point m.pos;velocity=copy_point m.velocity}

let to_position_array s=
  let lines=List.map String.trim @@String.split_on_char '\n' s in
  let components=List.filter (fun x->String.length x>0)
    @@List.map (Str.global_replace (Str.regexp "\\([^-0-9]\\)") " ") lines in
  List.iter print_endline components;
  Array.of_list @@
  List.map (fun x->
      let l=
      List.map int_of_string @@ List.filter (fun x->String.length x>0) @@
      List.map String.trim @@
      String.split_on_char ' ' x in
      {x=List.nth l 0;y=List.nth l 1;z=List.nth l 2}
    ) components

let to_moons arr=
  Array.map (fun pos->{pos=pos;velocity={x=0;y=0;z=0}}) arr

let adjust_velocity (arr:moon array)=
  let rel_vel_adjust a b=
    if a==b then 0
    else
    if a<b then
      1
...
let ()=
  ...
  while not (!found_x && !found_y && !found_z) do
    step c;
    if not !found_x then begin
      xr:= !xr + 1;
      if Array.fold_left (&&) true @@ Array.map2 (fun x y->
          x.pos.x=y.pos.x&&x.velocity.x==y.velocity.x
        ) c q  then begin
        found_x:=true;
        end
    end;
    if not !found_y then begin
      yr:= !yr + 1;
      if Array.fold_left (&&) true @@ Array.map2 (fun x y->
          x.pos.y=y.pos.y&&x.velocity.y==y.velocity.y
        ) c q then begin
        found_y:=true;
      end
    end;
    if not !found_z then begin
      zr:= !zr + 1;
      if Array.fold_left (&&) true @@ Array.map2 (fun x y->
          x.pos.z=y.pos.z&&x.velocity.z==y.velocity.z
        ) c q  then begin
        found_z:=true;
      end
    end;
  done;
  Printf.printf "Determined cyclicity over %i steps at round %i\n" (lcm !xr (lcm !yr !zr)) (max !xr (max !yr !zr))
