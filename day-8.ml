...
let split_layers s n : string list =
  let rec collect l s =
    ...
    if String.length s = 0 then l
    else collect (String.sub s 0 n :: l) (String.sub s n (sl - n))
  in
  collect [] s
...
let find_min img c =
  let rec min_min best best_count i =
    ...
    | row :: rest ->
        ...
        if nr < best_count then min_min row nr rest
        else min_min best best_count rest
    | _ -> best
  in
  let c = count_occurences (List.hd img) c in
  min_min (List.hd img) c (List.tl img)

let rec first_n xs n =
  match xs with
  | [] -> failwith "First_n"
  | x :: xs -> if n = 1 then [ x ] else x :: first_n xs (n - 1)

let header output = output_string output "P1\n 25 6 \n"
...
let rec print_layer output layer =
  ...
  if sl > 0 then (
    ...
    @@ Str.global_replace (Str.regexp "2") "0"
    @@ String.sub layer 0 25;
    output_char output '\n';
    print_endline
...
let combine l1 l2 =
  let c = String.mapi (fun i x -> if x = '2' then l2.[i] else x) l1 in
  let n = !num in
  num := !num + 1;
...
let () =
  ...
  Printf.printf "Got %i rows of length %i\n" (List.length image)
    (String.length @@ List.hd image);
  let best_row = find_min image '0' in
  Printf.printf "%i\n"
  @@ (count_occurences best_row '1' * count_occurences best_row '2');
  let image = List.rev image in
  let final =
    String.map (fun x -> if x == '0' then ' ' else '1')
    @@ List.fold_left combine (List.hd image) (List.tl image)
  in
  for i = 0 to 5 do
    print_string
    @@ Str.global_replace (Str.regexp "1") "█"
    @@ String.sub final (i * 25) 25;
    Printf.printf " %i\n" ((i + 1) * 25)
  done;
  print_endline final
