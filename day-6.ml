type orbit=
  | Node of string * int * orbit array
  | Leaf of string * int
...
let rec to_graph t depth node=
  ...
  if List.length successors = 0 then
    ...
    Node (node, depth,
          Array.of_list
          @@ List.map (to_graph t (depth+1)) successors
         )
...
let rec augment_orbit (orbit:orbit):augmented_orbit=
  ...
  | Node (s,_,l)->
    ...
    let (a,b)=Array.fold_left (fun (x,y) (w,z)->
        (min x w),
        (min y z)) (not_found_const*2,not_found_const*2)
      @@Array.map cost l in
